...
func isPlainOperation(s *syntax.Statement) bool {
	...
	return len(s.Blocks()) == 0 && set("draw", "add", "summon", "damage", "heal", "buff", "gain", "restore", "destroy", "banish", "discard", "remove", "return", "evolve", "superevolve", "reanimate", "reduce", "halve", "spellboost", "transform", "set_attack_limit", "set_damage_reduction", "set")[s.Word(0)]
}
...
func strictOverride(s *syntax.Statement, ds *[]syntax.Diagnostic) {
	...
	case "evolved", "super_evolved":
		ok = ok && len(t) == 1
	case "ward", "storm", "rush", "bane", "drain", "intimidate", "barrier", "stealth", "aura", "ability_target_guard", "ability_destruction_guard", "cannot_attack", "cannot_attack_follower", "cannot_attack_leader":
		ok = ok && len(t) == 1
	case "cost", "earthsigil", "countdown", "damage_taken":
		ok = ok && len(t) == 2
		if ok {
			checkU16(t[1], true, ds)
		}
