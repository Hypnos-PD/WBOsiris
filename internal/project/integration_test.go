...
func TestCompileDeterministicAndStructured(t *testing.T) {
	...
	if len(typed.Cards) != 679 || len(typed.Sources) != 679 {
...
func TestCompileTestPackStructured(t *testing.T) {
	...
	if len(typed.Scenarios) != 1002 || typed.Scenarios[0].Name != "Orchis grants Storm and Bane to the summoned Lloyd" {
	...
	if len(scenarios) != 1002 {
		t.Fatalf("scenarios=%d", len(scenarios))
	}
	first := scenarios[0].(map[string]any)
	if first["name"] != "Orchis grants Storm and Bane to the summoned Lloyd" {
