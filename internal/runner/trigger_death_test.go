...
func TestDeathBatchMovesAllEntitiesBeforeQueueingLastwords(t *testing.T) {
	...
	if len(session.g.events) != 9 || session.g.events[6].BatchID == 0 || session.g.events[6].BatchID != session.g.events[7].BatchID || session.g.events[7].BatchID != session.g.events[8].BatchID {
