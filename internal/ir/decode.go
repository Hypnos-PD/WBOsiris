...
func decodeEffectShape(data []byte, nodeIDs map[string]bool) (Effect, error) {
	...
	case "mode":
		...
		if v.Count < 0 || v.Count > 65535 || v.Random && v.Count < 1 || v.History != "" && (!v.Random || !ValidBindingName(v.History)) {
			return nil, fmt.Errorf("invalid mode count")
		}
		e := ModeEffect{NodeBase: NodeBase{v.ID, v.Origin}, Kind: v.Kind, Count: v.Count, Random: v.Random, History: v.History}
		seen := map[int]bool{}
	...
	case "damage", "heal", "buff_stats", "destroy", "banish", "discard", "return", "add_keyword", "remove_keyword", "remove_ability", "silent_evolve", "set_attack_limit", "set_damage_reduction", "set_life", "set_cost", "set_attack":
		...
		case "set_attack_limit":
			if v.Amount < 1 || v.Amount > 65535 || v.DamageType != "" || v.Keyword != "" || v.Form != "" || v.Destination != "" || v.DeckInsertion != "" || v.AttackDelta != 0 || v.LifeDelta != 0 || p != nil {
...
func decodeCondition(data []byte) (Condition, error) {
	...
	if k.Kind == "attack_history" {
		...
		return AttackHistoryCondition{Kind: v.Kind, Side: v.Side, Attacked: *v.Attacked, LeaderLastTurn: v.LeaderLastTurn}, nil
	}
	if k.Kind == "self_form" {
...
func validKeyword(v string) bool {
	return oneOf(v, "ward", "storm", "rush", "bane", "drain", "intimidate", "barrier", "stealth", "aura", "ignore_ward", "ability_target_guard", "ability_destruction_guard", "damage_taken_up", "damage_to_zero", "cannot_attack", "cannot_attack_follower", "cannot_attack_leader")
}
