...
改写成不成功，判据是 broker 那边能不能解开：**能解开的请求会打出"信封：解出 N 字节
明文（凭据自检 head=true value=true）"**。实测（1.9.5 客户端）第一次出现这句是在
`POST /cygames/Title/index`，之后客户端能正常走到标题界面。

仍然解不开的是每次启动最早的那个 `POST /cygames/CrashLog/send`：它比改写生效还早，
用的是原公钥。这不是故障，broker 会如实报告，客户端也不依赖它的响应。

监视的代价是启动后约 15 秒内每轮扫一遍地址空间（20 轮封顶）。这一段可以再优化：
元数据出现的时间点是确定的（比第一个请求早一点），没必要固定扫满。

> 插一句教训：曾经有一轮把"载荷认证失败"归因到公钥改写不生效，查了很久内存。
> 真正的错在信封实现里（`mix` 用了 `mixed[0]`，参考实现是 `mixed[i]`）。判据其实
> 就在报文里：**元数据那一步只用到 shared**，它的凭据片段对不上才是改写的锅。
> 现在 broker 的报错会直接区分这两种情况。
