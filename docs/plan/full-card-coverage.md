...
| S-30 | ~~牌组替换与牌组底部变身~~ **已解决**：`replace own\|oppo.deck with shuffled …` + `set empty deck own\|oppo victory;` | 已解锁 10304110 绝大的显现·麦哲佩恩 | 牌组替换早已实现；"牌组底部的亡者之王卡牌变身为胜利的卡牌"实现为"牌组耗尽时改为胜利"（`empty_deck_outcome`，跨续局保存），与 SWB-RL 的 `set_empty_deck_outcome` 同一口径。 |
...
| S-35 | ~~信仰值（faith）机制~~ **已解决**：`faith { counter value 0; … }`、`faith N { … }`、`distribute faith <卡牌ID> { … }`、`grant faith { … }`、`grant faith modes N` | 已解锁 10634120 古旧天晶、10664120 古旧天书、90034330 天晶深渊、10614120 古旧天枪、10624120 古旧天剑、10354110 混融的继承者 | 信仰实体与纹章共用结构/区域，开局按初始牌组放置；信仰值用 `counter value`；`faith N { … }` 是"信仰值-N 后执行"的支付块（不足则整块跳过）；`distribute faith` 把信仰值逐点随机分配（不消费）；`grant faith { … }` 给信仰附加任意事件监听；`grant faith modes N` 给信仰的 `mode_bonus` 计数器加成，玩家选择【模式】时数量随之增加（追加 `when own\|oppo mode selected` 事件）。 |
...
| S-39 | ~~动态随机/选择数量~~ **已解决**：`random … count <数值表达式>` | 已解锁 10373110 破坏的团结者、10364110 安息的继承者·妃花；10811110 的"两个计数相减"由 S-41 的减法补齐 | `SelectionEffect` 新增 `CountExpr`，运行时在结算到该语句时求值（0 表示不选、超过候选数按候选数截断）；续局恢复沿用挂起请求里保存的数量。 |
| S-41 | ~~数值算术（减法）~~ **已解决**：`<数值表达式> - <数值表达式>` | 已解锁 10811110 昔日的天秤·马龙 | 新增 `DifferenceExpr`（`kind: "difference"`，左右都是数值表达式，自左向右结合）；字面量不能当操作数（`count(A) - 1` 在检查阶段报错），负结果由取用方按零处理。加法暂时没有卡牌需要。 |
...
| S-60 | ~~按牌组随机随从变身并复制~~ **已解决**：`transform <目标> into random card from <集合>` | 已解锁 10533310 壮美的明越花 | S-93 的随机复制变身正好覆盖"分别变身为牌组中随机 1 张随从的复制"：每个目标各自抽一次，只读卡牌定义。 |
...
| S-92 | ~~从对手手牌/牌组随机复制卡牌~~ **已解决**：`add random N copies from <集合> to hand\|deck` 接受任意集合 | 已解锁 10802310 救世的英姿、10912120 温泉度假猴、10514110 脚踩天穹的《倒吊人》·罗弗拉德 | 复用 S-64 的抽选规则（无放回、每条候选消费一次随机决策），新的 `copy_random` 节点只读卡牌定义并创建新实例，不移动来源、不产生带来源身份的事件，因此天然是"非公开"加入手牌。 |
...
| S-74 | ~~逐个记住"尚未发动"的模式能力~~ **已解决**：`mode random N history <名字>` | 已解锁 10574110 转动的《命运之轮》·斯洛士 | 实例新增 `modeHistory`（记录名 → 已发动选项集合），随机模式先排除已发动选项，再记录本次发动；跨回合与续局保留，全部发动过时该模式不产生效果。 |
...
| S-76 | ~~再次发动自身【入场曲】~~ **已解决**：`replay fanfare self;` | 已解锁 10604110 恐惧的象征·欧米伽奥提普 | 运行期按实例当前的卡牌定义重新执行 `fanfare` 能力（新的效果帧，重新随机）；`fanfareReplays` 计数随实例快照保存，上限 **20 次**（官方 QA 0vrc9nkxfkx1：入场曲最多 21 次、+4/+4 最多 21 次）。 |
| S-77 | ~~"通过【爆能强化】使用卡牌时"事件~~ **已解决**：`where enhanced` | 已解锁 10622310 威风的行军 | 打出事件现在带上"本次支付了爆能强化档位"的标记，筛选词条 `enhanced` 读取它。 |
| S-51 | ~~主战者临时"受到的伤害变为 0"~~ **已解决**：`damage_to_zero` 主战者关键词 + 期限 | 已解锁 10444120 世界的伙伴·佐伊 | 主战者级关键词新增独立到期表（`leaderTemporary`），`add <关键词> to own\|oppo.leader until own\|oppo turn ends;` 在对应一方回合结束时清除；新关键词 `damage_to_zero` 让主战者受到的 1 点以上伤害变成 0（不消耗【屏障】）。测试 DSL 新增 `own.leader has/lacks <关键词>` 断言。 |
| S-52 | ~~牌组中发动与【瞬念召唤】~~ **已解决**：`when … while self in deck` + `invoke self;` + `when self invoked` | 已解锁 10404110 天司长的继承者·圣德芬、10904110 断绝的轮回·泽勒尔 | 牌组里的监听只允许回合开始/结束两个时点（`SourceZone: "deck"`），触发索引把牌组纳入扫描并按区域二次校验；`invoke` 把牌组实例移到战场（不付费、不计连击、不发动入场曲），随后派发 `card_invoked` 事件。同时新增玩家计数器 `own.evolutions`（本场对战随从进化次数，含超进化）。 |
| S-97 | ~~回合开始时"牌组中发动"与场上吟唱谢幕曲的先后~~ **已解决** | 10404110 与【吟唱_1】护符同时在场时的结算顺序 | 官方 QA：场的倒计数破坏 → 瞬念召唤 → 该护符的谢幕曲 → 返回手牌 → 回合开始的抽牌。现在回合开始阶段先用 `triggerSink` 收集倒计数破坏产生的触发（已破坏监听与谢幕曲），再入队牌组/手牌/战场的回合开始能力，最后追加收集到的触发，因此顺序与 QA 一致；有 2 个判别场景（`tests/10004/batch-108-invoke.wbotest`）。 |

`DrawEffect.Owner` 与执行器（`g.playerForSide(self, e.Owner)`）本来就支持任意一方，缺的只是语法入口，所以这次扩展只动了验证与解析两处，没有改运行时。

## QA 全量核查

`../WBArts/data/official_qa.json` 的 102 条简中 QA（其余语言是同一批问题的翻译，逐卡条数完全一致）
已逐条对照实现，结论与落地如下。判定顺序：官方卡牌文本 + QA > `../SWB-RL`（第二意见）。
...
| et1amf0xg7k / hz0p-10ss / 494pxxx2n | 回合开始：倒计数破坏 → 牌组中发动（瞬念召唤）→ 谢幕曲 → 返回手牌 → 抽牌 | `advanceTurn` 用 `triggerSink` 收集倒计数破坏的触发，排在牌组/手牌/战场的回合开始触发之后；2 个场景 |
| eilph_9kmewn | 被「牌组减费」的卡被瞬念召唤时按原始费用离开牌组 | `instance.deckCostBase` + `invokeInstance` 还原；Go 单测断言 8 费卡减半后瞬念召唤回到 8 |
| js2aw5l126-8 | 「受到的伤害 +1」重复获得会重复生效 | `player.leaderDamageTakenUp` 层数（含双方同时受伤的路径与快照）；Go 单测断言 +2 |
| mxtwkh_pd | 失去能力不影响已经入队的触发 | `removeAbility` 不再从队列里剔除已排队触发；Go 单测覆盖 |
| npml0tn71d | 剩余能量点够付原费用时不能【激奏】 | `accelerate` 预检新增 `pp < 当前费用`；场景断言非法 |
| qrpt1xyqmvgf | 一次选择【模式】只算一次（多选也只 +1） | `mode_selected` 改为每次选择动作派发一条；场景改用『叫唤与憎恶』双选断言 +1 |

### 逐条结论（102 条）

| QA | 涉及卡牌 | 结论 | 落地 |
| --- | --- | --- | --- |
| 0stcr3cyb | 10464110 土之法则·伽莱翁 / 10564110 思念的《力量》·索菲娜 | 一致：回合结束能力的条件在按下回合结束按钮时判定 | 卡面条件上移到触发器（`when own turn ends if …`）；场景「回合结束时的进化不满足同一时点锁定的条件」 |
| 0vrc9nkxfkx1 | 10604110 恐惧的象征·欧米伽奥提普 | **纠偏**：重发入场曲上限 12 → 20（+4/+4 共 21 次） | `fanfareReplayLimit = 20`；Go 单测 `TestFanfareReplayIsCappedAtTwenty` |
| 10nvv2q1zaop | 10001120 叮当天使·莉亚 / 10341110 侮蔑的肯定者 | 一致：交战受到的伤害只要没被破坏就发动（自己的回合） | — |
| 1sgy2e9--o | 10404110 天司长的继承者·圣德芬 / 90011110 妖精 | 一致：每次随机在剩余目标中等概率挑选（含主战者） | — |
| 204aw4wcz | 10851120 可爱恶魔·莉莉姆 | 一致：双方主战者同时归零时回合外玩家获胜 | — |
| 2e2l68z79011 | 10333110 真理的团结者 | 一致：复制体保持【超进化】状态 | — |
| 2hv4yt1j8 | 10224120 雷维翁超越者·尤里乌斯 | **纠偏**：结算时已经离开战场的来源，其已入队触发不再发动 | `triggerInvocation.zone` 校验；Go 单测 `TestQueuedTriggerIsDroppedWhenSourceLeavesField` |
| 2mrx5ewte1 | 10162210 禁密的圣地 / 10361110 安息的肯定者 | 一致：攻击力可以为负值参与计算（显示不为负） | — |
| 2ynq-vbwu | 10304110 绝大的显现·麦哲佩恩 | 范围外：牌组构筑规则（麦哲佩恩牌组） | — |
| 2z2966_-202 | 10374120 奏绝的显现·莉洁纳 / 90074310 奏绝的独唱 | 一致：选中不可破坏的卡时后续伤害照常发动 | — |
| 3txqy3qikdb | 90011110 妖精 / 90021120 铁甲骑士 | 范围外：卡牌收藏 UI（附属卡牌的类型检索） | — |
| 494pxxx2n | 10214110 翅翼女王·提泰妮娅 / 10404110 / 90011110 | 一致：纹章回合开始 → 瞬念召唤 → 返回手牌 → 回合开始抽牌 | 场景「纹章的回合开始能力先于瞬念召唤」 |
| 49odoxq3_z | 10473110 向往天空的回归者·卡西乌斯 | 一致：没有创造物随从时照样打出并造成 0 点 | — |
| 4fwjurcr30 | 10804110 阿尔比昂巴哈姆特 | 一致：信仰不属于纹章，不被「使所有纹章消失」消灭 | — |
| 4gt8z-3dnxk | 10152110 干练的死神·蜜诺 / 10554110 纹章：充实的恋人 | **纠偏**：爆能强化被纹章压制时按基础费用支付 | `playCost`/`commitPlay` 跳过爆能档；场景「纹章压制爆能强化时按基础费用支付」 |
| 4jmiujpae | 10304120 涸绝的显现·吉尔内莉莎 / 90004330 | 一致：需要双方最大能量点均为 10 | — |
| 4vvbwaghp | 10113130 煌击战士·贝鲁 / 10404110 | 一致：返回手牌算「离开战场」，贝鲁的减费发动 | — |
| 5d3h_y1sc | 10101110 贪婪的智天使·露比 / 10131120 见习占星术师 | 一致：返回牌组的卡可以被再次抽到，返回位置随机 | — |
| 5hx3x497q3ub | 10314110 不弑的继承者·库露露 / 10813310 水镜的信赖 | 一致：卡牌本体效果先结算，纹章的触发后结算 | — |
| 6fl_t5sbh9-5 | 10432120 荆棘旅途·米蕾耶&莉赛特 | 一致：两次进化各给手牌奥义槽 +1 | — |
| 6v0zf6_ym | 10354110 混融的继承者·莎木·纳克雅 | 一致：信仰值不足 10 时整块跳过、不扣信仰值 | — |
| 6wo09701r | 10303110 充满勇气之人 | 一致：只对「对手的随从」超进化给【疾驰】 | — |
| 75wdwp8ysf | 10522120 吉祥蛙 / 10524120 / 90021350 | **纠偏**：卡面「若…则…」上移到触发器，避免回合结束阶段后加入的牌补足条件 | 场景「回合结束时加入的法术不参与同一次回合结束的条件判定」 |
| 7g3t2088i | 10413110 幻彩弓手·丘比丹 / 90051140 腐臭的僵尸 | 一致：处理途中新入队的触发不插队 | — |
| 7pm0futqc | 10233110 玛纳利亚剑士·欧文 / 10472110 | 一致：【交战时】攻击方先结算 | — |
| 7wzi4bb1a | 10533310 壮美的明越花 / 10534110 | 一致：变身不触发「进入战场时」的能力 | — |
| 85ime-6qtj-j | 10354110 / 10624120 古旧天剑·伊德梅塔 | 一致：信仰按卡牌 ID 绑定，别名的信仰值不算 | — |
| 9296yevdkb3a | 10423310 骁勇骑士 | 一致：攻击后再提高攻击次数上限可以追加攻击 | — |
| 9bd9w189q | 10214120 缠绕密林·丽梅格 | 一致：超进化随从在该用户回合内受到的伤害为 0（0 伤害也算「受到伤害」） | — |
| 9iec66iq2 | 10404110 / 10574110 转动的《命运之轮》·斯洛士 | 一致：纹章的回合开始能力先于牌组中的回合开始能力 | — |
| 9jbr88t6fy6j | 10572310 苏生调律 | 一致：每张卡等概率（同名多张更易被选中） | — |
| a7wdp3yujp | 10344120 烈绝的显现·嘉尔缪 | 一致：只在自己的回合发动 | — |
| a94bewhaov | 10363110 安息的团结者 | 一致：本回合是否有随从攻击过按玩家记录，不看破坏后是否在场 | — |
| bxi2b429pl7 | 10172320 改境的重启 / 10274120 / 90044310 | 一致：按当前费用筛选；法术没有可选目标时不能使用 | — |
| c_91v-gas | 10412110 美妆少女·克洛伊 | 一致：效果召唤不发动【入场曲】 | — |
| ci27dr9dve | 10313110 不弑的团结者 / 10414120 | 一致：每回合一次的纹章只让最初使用的那个进化 | — |
| ckrd5um859yo | 10314110 / 10554110 | 一致：「使用～时」先于「～进入战场时」（与获得纹章顺序无关） | — |
| ct9bsy6n4 | 10001110/10001130/10002120/10341310 雷霆之怒 | 一致：「生命值最大」按当前生命值（含受伤的），同值全部命中 | — |
| dkhkfbz1hu | 10404110 | 一致：只有瞬念召唤才获得纹章 | — |
| dt7oehzyd | 10104110 勇武的堕天使·奥莉薇 | 一致：能力引起的进化不发动【进化时】/【超进化时】 | — |
| dx_f3f8kygrp | 10503210 大游戏世界 | 一致：每次打出最多让倒计数 -1 | — |
| dzyifirn1cll | 10804120 高洁的黑翼·奥莉薇 | 一致：进化点/超进化点上限为 2 | — |
| e03skkdj6s6 | 10624110 惨烈的剑王 | 一致：支付到 8 点时 7 点档的能力也发动 | — |
| eilph_9kmewn | 10244120 绚丽凤凰·小凤 / 10404110 | **纠偏**：瞬念召唤按原始费用离开牌组，返回手牌的费用回到 6 | `instance.deckCostBase` + 瞬念召唤还原；Go 单测 `TestInvokeRestoresDeckScopedCost` |
| et1amf0xg7k | 10163210 野兽公主的誓约 / 10404110 / 90061110 | **纠偏**：倒计数破坏 → 牌组中发动（瞬念召唤）→ 谢幕曲 → 返回手牌 → 抽牌 | 回合开始阶段的触发收集（`triggerSink`）；场景「倒计数破坏的谢幕曲排在牌组中发动的瞬念召唤之后」 |
| fa6vssiw950a | 10621110 勇烈的士兵 / 90044310 | 一致：费用被 +1 不影响【爆能强化_3】发动 | — |
| fmhlfq2lj | 10552310 残虐的炸裂 | 一致：只打击当前生命值最小的主战者 | — |
| fw6g86umau59 | 10244120 / 10403120 露莉亚 | 一致：【爆能强化_8】参照当前费用 | — |
| g-n1z6qpqqw | 10362210 安息的神殿 / 10474120 别西卜 | 一致：「受到伤害+1」与【屏障】并存时下一次伤害为 0 | — |
| g1qiwt790 | 10204120 / 10364120 | 一致：纹章的回合结束能力按获得的先后顺序发动 | — |
| gi6eaipmt3m | 10133320 唤鬼术 / 10503210 | 一致：大游戏世界参照原始费用 | — |
| gj6r99bao6d6 | 10204110 命运黄昏·奥丁 | 一致：【土之印】不能被对手的能力选中 | — |
| gzffm2_g3y | 10133320 / 10134310 超越次元 | 一致：返回牌组保留魔力增幅（费用变化随实例保留） | — |
| h3225qzf5 | 10624120 | 一致：信仰被授予两次时，一次爆能强化发动两次强化 | Go 单测 `TestFaithGrantStacksAcrossRepeatedEvolutions` |
| h4y64az4e | 10514110 脚踩天穹的《倒吊人》·罗弗拉德 | 一致：按已进化时的对手牌组抽选 | — |
| h6gjpnexd | 10153140 / 10444120 | 一致：「伤害变为 0」优先于纹章的回合结束伤害 | — |
| h8v47in7_4mk | 10372110 破坏的祈祷者 / 10374120 | 一致：不可被破坏的目标不影响后续随机伤害 | — |
| hz0p-10ss | 10161210 祥和的教会 / 10404110 | **纠偏**：同 et1amf0xg7k（谢幕曲抽牌排在瞬念召唤之后、回合开始抽牌之前） | 同上场景 |
| ie_ps9k893 | 10401120 / 10414120 / 90011110 | 一致：每回合一次在触发入队时消耗（第一张因自身能力进化也算用过） | — |
| js2aw5l126-8 | 10474120 | **纠偏**：「受到的伤害 +1」可叠加 | `player.leaderDamageTakenUp` + 快照；Go 单测 `TestLeaderDamageTakenUpStacks` |
| jwgzs064sln | 10363210/10364120/10514120/10714120/10753310/90024310 | 一致：从更早出场的随从开始按生命值等量分配，最后溢出给主战者 | — |
| k6l1dc0-v | 10123110 雷维翁之斧·杰诺 / 10472110 | 一致：【攻击时】先获得【屏障】，交战时消耗屏障，之后照常吃战斗伤害 | — |
| k7-7gbpxew0 | 90051140 / 90073130 毁灭创造物γ | 一致：回合结束伤害整批结算完后才轮到谢幕曲召唤 | — |
| kah2ug6tj69l | 10101110/10143110/10244120 | 一致：小凤只影响发动时在牌组里的卡 | — |
| l0dv1pxb7 | 10804110 | 范围外：卡牌插画 | — |
| lbe2tcmqi4 | 10144110 | 范围外：回放 UI；引擎侧在生命归零时已结束对局 | — |
| lm9y3xju9y0z | 10131310 彩虹奇迹 | 一致：需要选择目标的法术没有可选对象时不能使用 | — |
| lqavg6h-h | 10451310 / 10454110 | 一致：纹章的回合结束能力先于战场上卡牌的回合结束能力 | — |
| m5vf5p8a4jd | 10552310 | 一致：双方生命值相同时都受到伤害 | — |
| m8n7gz5kol5z | 10224110 静寂的安纳提玛·吉尔达利娅 | 一致：手牌打出的随从在结算后才计入协作 | — |
| mdvnb17c15 | 10124110 | 范围外：卡牌收藏（制作闪卡） | — |
| mxtwkh_pd | 10224120 / 10474120 | **纠偏**：失去能力不影响已经入队的触发，处理途中的能力继续结算 | `removeAbility` 不再剔除已排队触发；Go 单测 `TestRemovingAbilitiesKeepsQueuedTriggers` |
| noxkybph23d4 | 10574110 | 一致：纹章先于回合开始的规则抽牌 | — |
| npml0tn71d | 10672110 拙劣的人偶 | **纠偏**：剩余能量点够付原费用时不能用【激奏】 | `accelerate` 预检加条件；场景「剩余能量点够付原费用时不能激奏」 |
| o-5wc8a4g0jf | 10514110 | 一致：5 次抽选无放回，同名多张提高概率 | — |
| o6pjp6ssc5ek | 10244120 / 10543110 破灭屠戮者 | 一致：【超进化时】参照当前费用 | — |
| o74bzd_mnn | 10154120 / 10472110 | 一致：攻击时破坏交战对手后不再受到其【交战时】伤害 | — |
| o_h1vs_xxx | 10533110 元素支配者 / 90031120 | 一致：每个入场的巨像各自支付【土之秘术】，不足的跳过 | — |
| okejcr1akzjh | 10001130/10403120/10673110 愚劣的兵器 | 一致：每个实例各自判断分配伤害目标 | — |
| oye9bi1im | 10011110 妖精驯服者 / 10314110 | 一致：【入场曲】先于纹章的「进入战场时」 | — |
| p6pyv8_szlbl | 10354110 | 一致：使用两次入场曲让「模式选择数 +1」叠加到 +2 | — |
| q0bo1yysvucb | 10163110/10363110/10364110/10364120 | 一致：按张数等概率（同名多张更易被选中） | — |
| qn9tzuv35 | 90054110/90054120 | 一致：指定费用以内取费用最大的那些再随机，同名多张提高概率 | — |
| qrpt1xyqmvgf | 10353310 叫唤与憎恶 / 10354110 | **纠偏**：一次选择【模式】只算一次（即使同时选中多个模式） | `mode_selected` 改为每次选择动作一条；场景「一次选择两个模式只让信仰值加一」 |
| r4jiyc9zoino | 10134110 五行之巅·久苑 / 90034120 式神·贵人 | 一致：按本回合被破坏的式神的原始攻击力/生命值合计加成 | — |
| r6iwd8iutw | 10161210 / 10574110 | 一致：纹章先于战场卡牌；谢幕曲抽到的牌不受先前的费用 +1 影响 | — |
| sbpk3ev2pck6 | 10131320/10831310 暴风破 | 范围外：卡牌分解流程 | — |
| ssipqu3d6vv0 | 10151310 死神挥刀 | 一致：超进化随从在该用户回合内不会被能力破坏 | — |
| t_6kpw1s00g | 10562120 穷途末路的巫女 | 一致：手牌满时抽到的卡被舍弃，不触发「自己抽取卡牌时」 | — |
| tlqrrpwbeec6 | 10621110/10623110 | 范围外：私人对战中的禁限卡规则 | — |
| u8htn50do0yx | 90034330 天晶深渊 | 一致：每一点信仰独立等概率分配到 X/Y/Z | — |
| ub1lxzl1ns | 10664110 崇高的憎恶·康蒂玛 | 一致：按破坏历史等概率（同名多张提高概率） | — |
| utg7xrt615a | 10444120 / 10474120 | 一致：「伤害变为 0」让最终的伤害 +1 也变成 0 | — |
| v-6amcbgd2vc | 10001130 / 10344120 | 一致：伤害被减为 0 也算「受到伤害且没被破坏」 | — |
| vct0y4ai1p3t | 10314110 / 10344110 | 一致：被纹章破坏后，已入队的「受伤存活」触发不再发动 | — |
| we8jky5pa | 10214110 / 10574110 / 90011110 | 一致：纹章按获得顺序发动，后加入手牌的妖精才会被 +1 | — |
| wqp9s3-6r | 10474120 | 一致：主战者被攻击时按攻击力 +1 受到伤害 | — |
| yh45wq1d_p | 10304120 / 90004330 | 一致：额外能量点不改变最大能量点，双方都为 10 才给甘露 | — |
| ymeqm0d8z | 10212120 | 范围外：卡牌收藏（制作闪卡） | — |
| yxjd4gfl8m | 10001130/10103310/10503210 | 一致：按原始费用比较，同原始费用的卡存在即 -1 | — |
| z33ajxb0p8q | 10001120 / 10344120 | 一致：交战中受到伤害且存活即发动 | — |
| zov-4dbr6tlx | 10461120 克己复礼的修女·拉姆蕾达 / 10464110 | **纠偏**：回合结束阶段被进化不满足「按下结束按钮时」锁定的条件 | 卡面条件上移到触发器；场景「回合结束时的进化不满足同一时点锁定的条件」 |

### 与 QA 无关的已知偏差（未处理，记录在案）

- 回合结束能力里的 `if … else …` 分支（10544110、10524110、10153110、10742120、90014320、10703210、10863210）
- 以及带「之后，若…」的 10553310：QA 只明确了「能力是否发动」在按下回合结束按钮时判定，这些卡的分支选择仍按结算时求值，属于刻意保留的简化。
- 非引擎范围（卡牌收藏/分解、私人对战禁限卡、牌组构筑、回放 UI、卡牌插画）共 8 条：2ynq-vbwu、3txqy3qikdb、l0dv1pxb7、lbe2tcmqi4、mdvnb17c15、sbpk3ev2pck6、tlqrrpwbeec6、ymeqm0d8z。

## 批次记录

### 批次 122（QA 全量核查与纠偏）

- **范围**：`../WBArts/data/official_qa.json` 的 102 条简中 QA（其余语言是同一批问题的
  翻译，逐卡条数完全一致）。逐条对照卡牌规则与引擎语义，结论见 [QA 全量核查](#qa-全量核查)。
- **本轮纠偏的 10 处**：重发入场曲上限（12 → 20）、模式选择事件计数、主战者「受到伤害 +1」可叠加、
  触发来源离开战场后不再发动、失去能力不影响已入队触发、纹章压制爆能强化时按基础费用、
  瞬念召唤按原始费用离开牌组、回合开始的"倒计数破坏 → 牌组中发动 → 谢幕曲 → 返回手牌 → 抽牌"顺序、
  回合结束条件在按下按钮时判定（18 张卡的条件上移到触发器）、【激奏】的能量点前提。
- **测试**：新增 5 个场景与 5 个 Go 单测（`replay_fanfare_limit_test.go`、
  `field_leave_trigger_test.go`、`damage_cap_test.go` 的叠加用例、`invoke_cost_test.go`、
  `faith_grant_stack_test.go`）。
- 全量回归：`check` 0 错 0 警（910 张卡 / 1496 个场景）；`test` 1496 全绿；
  `go test -count=1 ./...` 全绿；`node scripts/card_scope_screen.mjs --verify` 退出 0；
  `node --test scripts/` 55 项全绿。
- 续局不再需要向后兼容：回合开始阶段合并后去掉了 `starting_triggers` 这一步。
...
- **`set empty deck own|oppo victory|defeat;`**：改变该玩家牌组耗尽时的结果，
  对应纹章"使自己的牌组底部的亡者之王卡牌变身为胜利的卡牌"（与 SWB-RL 的
  `set_empty_deck_outcome` 同一口径）；玩家字段随续局保存。
- **`where not card <卡牌ID>`**：筛选取反新增 `card` 词条，用于"舍弃除『绝大的证明』以外的所有手牌"。
- 完成 1 张：10304110 绝大的显现·麦哲佩恩（入场曲加入『绝大的证明』；进化时获得纹章：
...
### 批次 115（S-35 收尾：信仰的"模式选择数 +1"）

- **`grant faith modes N;`**：给该玩家信仰上的 `mode_bonus` 计数器加 N（信仰定义里声明
  `counter mode_bonus 0;`），玩家选择【模式】时可选数量随之增加，超过选项数时截断；
  只影响玩家选择，不影响 `mode random`；信仰消失时加成随之消失。
- **新事件 `mode_selected`**：玩家确认模式选择后派发一次，写作
  `when own|oppo mode selected { … }`（`random` 模式不派发）。最初按每个选中的选项各派发一次，
  本轮按官方 QA qrpt1xyqmvgf 纠正为"每次选择动作一次"。
- 完成 1 张：10354110 混融的继承者·莎木·纳克雅（入场曲：信仰值-10 并使信仰获得
  "自己选择的【模式】数+1"；超进化时破坏选中的敌方随从并把同名卡加入手牌；
  信仰本体：自己选择【模式】时信仰值+1）。
- 新增 4 个场景（`tests/10003/batch-115-faith-modes.wbotest`），覆盖信仰值消耗与加成、
  信仰值不足整块跳过、超进化复制、加成让玩家多选一个模式并因此让信仰值 +2。
- 全量回归：`check` 0 错 0 警；`test` 1412 全绿；`go test ./...` 全绿；语料快照更新为 1412 个场景。
...
- **纹章 `passive` 语句**：`passive suppress_fanfare;` / `passive suppress_enhance;` 表示
  持有者的随从入场曲／爆能强化不发动。派生纹章实例（`CrestCard()`）会带上这些被动，
  运行期由 `playerPassive` 查询；`commitPlay` 在结算打出的效果帧时跳过被压制的能力。
  "自己使用的随从入场时进化"不需要新原语，用 `when own card played where type follower
  { evolve played silent; }` 表达（与既有纹章写法一致；效果引起的进化不发动进化时能力）。
- 完成 1 张：10554110 充实的《恋人与节制》·米路缇欧&卢泽（入场曲发动【亡者召还_4】与
  【亡者召还_2】各一次；进化时破坏战场上随机 6 个其他随从；超进化时获得
  "随从入场曲与爆能强化不发动、使用的随从立即进化"的纹章）。
- 新增 7 个场景（`tests/10005/batch-113-passive-crest.wbotest`），覆盖两次亡者召还、
  进化破坏 6 个、超进化给纹章、纹章压制入场曲／爆能强化并自动进化，以及无纹章时的对照。
- 全量回归：`check` 0 错 0 警；`test` 1405 全绿；`go test ./...` 全绿；语料快照更新为 1405 个场景。

### 批次 112（S-74 逐个记住"尚未发动"的模式能力）

- **S-74 `mode random N history <名字>`**：实例新增 `modeHistory`（记录名 → 已发动选项集合）。
  随机模式先排除该实例已发动过的选项，再从剩下的里等概率选；本次发动的选项随即记录，
  跨回合与续局保留；全部选项都用过时该模式不产生效果、也不消费随机决策。
  纹章里的记录属于纹章实例，与来源卡牌实例互不影响。
- 完成 1 张：10574110 转动的《命运之轮》·斯洛士（【潜行】；自己的回合开始时从三个未发动
  能力中随机发动 1 个；进化后自己的回合结束时让对手获得【吟唱_3】的负面纹章并使本随从消失）。
...
### 批次 111（S-76 再次发动自身【入场曲】）

- **S-76 `replay fanfare self;`**：按实例当前的卡牌定义查找 `fanfare` 能力，作为新的效果帧
  重新执行（其中的 `mode random` 会重新随机）。同一实例的重发次数记录在 `fanfareReplays`
  并随实例快照保存，上限 20 次（官方 QA 0vrc9nkxfkx1 给出的是规则的真正上限：
  入场曲最多 21 次、+4/+4 最多 21 次。最初写成 12 只是防止随机自引用无限递归的
  权宜值，本轮按 QA 纠正）。
- 完成 1 张：10604110 恐惧的象征·欧米伽奥提普（入场曲从 4 个能力中随机发动 2 个：
  随机破坏敌方随从／对对手主战者 2 点／回复 2 点能量点／+4/+4 并重发入场曲；
...
- **主战者级关键词的期限**：`player.leaderTemporary` 记录到期时点，
  `add damage_to_zero to own.leader until oppo turn ends;` 在对手回合结束时清除，
  不影响同一位主战者的其他关键词（随从的临时关键词逻辑保持原样）；
  续局快照一并保存。
- **新关键词 `damage_to_zero`**：主战者受到的 1 点以上伤害变为 0（0 点仍是 0），
  在"伤害 +1"之后、"【屏障】"之前应用，因此归零不会消耗【屏障】。
- **测试 DSL 新增** `own.leader has <关键词>` / `own.leader lacks <关键词>` 断言
  （`KeywordAssertion` 新增 `leader_keyword`），用于验证主战者关键词与其到期。
- 完成 1 张：10444120 世界的伙伴·佐伊（入场曲：能量点最大值 +1；爆能强化 10：
...
- **S-52 三件套**：`when … while self in deck { … }`（只在回合开始/结束生效）、
  `invoke self;`（把牌组里的实例移到战场：不付费、不增加连击、不算抽牌、不发动入场曲，
  但产生正常入场事件，随后派发 `card_invoked` 事件）、`when self invoked { … }`
  （"被【瞬念召唤】时"）。
  触发索引纳入牌组并在触发时按 `sourceZone` 二次校验，避免移动后的残留索引误触发
  （顺带修好手牌监听离开手牌后仍可能命中的隐患）。
- **`own.evolutions` / `oppo.evolutions`**：本场对战中该玩家随从进化过的次数（含超进化），
  跨回合与续局保存，测试状态可用 `evolutions 6;` 直接摆出。
- 官方 QA 落实三点：① 只有瞬念召唤才获得纹章（正常打出不给）；② 瞬念召唤后本卡牌
  返回手牌；③ 纹章的回合开始能力先于牌组中发动的能力结算。当时"场上吟唱谢幕曲与
  瞬念召唤的相对顺序"与 QA 不同（S-97），本轮已按 QA 修好（见 QA 全量核查一节）。
- 完成 2 张：10404110 天司长的继承者·圣德芬（在牌组中发动：回合开始且进化过 6 次以上时
  瞬念召唤→获得纹章→返回手牌；纹章【吟唱_2】回合结束回复全体随从与主战者各 1；
  【入场曲】【解放奥义】5 次随机 2 点）、10904110 断绝的轮回·泽勒尔（在牌组中发动：
  回合结束且费用 1..8 都用过时瞬念召唤；入场曲 9 点；【威慑】）。卡包 10009 因此收满。
...
- **S-95 `add 1 counter NAME modulo M`**：计数器加上增量后按 M 取模，用于"按顺序发动
  以下1个能力"这类走完一轮回到第一个的写法。`AdjustEffect` 新增 `modulo`（编解码与
  运行期同步），不带 `modulo` 的旧写法不受影响。
- **S-60 随机复制变身**：`transform <目标> into random card from <集合>` 改成
  **每个目标各自抽一次**（"分别变身为…的复制"），候选集合只查询一次；
  10602210 只有单目标，行为不变。顺带把变身逻辑抽成 `transformTarget`。
- 完成 2 张：10964120 翼天的变貌·奥梅里欧（自己的护符被破坏时按顺序发动三个能力：
  随机 2 个敌方随从各 3 点／回复主战者 2 点／召唤神圣猎鹰；进化时破坏自己的所有护符）、
  10533310 壮美的明越花（自己的所有随从分别变身为牌组中随机 1 张随从的复制）。
...
- **S-94 `first <绑定> from <集合> [where …] [count N]`**：新的选择语句，按区域顺序
  （战场=入场顺序、手牌=获得顺序）取前 N 个，`count` 省略时为 1。既不询问玩家、
  也不消费随机决策，候选不足时只取现有的，空集合绑定空集合；与 `random` 一样
  不经过"敌方潜行/不可选中"那类目标保护判定。IR 用已有的 `SelectionEffect`
  加 `kind/policy: "first"` 表示，续局不会产生挂起请求。
- 完成 2 张：10423310 骁勇骑士（4 个模式：最左皇家护卫随从每回合攻击 2 次／全体皇家护卫
  +1/+1 且【屏障】／回复 2 点能量点与 1 点进化点／回复主战者 6 点）、
  10502110 星辉女神（进化时舍弃 3 张手牌，再把剩余手牌最左 3 张各复制 1 张加入手牌）。
...
- **S-92 `add random N copies from <集合> to hand|deck`**：原来的写法只接受破坏历史，
  现在也接受任意集合（对手手牌、对手牌组、自己牌组……）。新 IR 节点 `copy_random`
  复用 S-64 的抽选规则：无放回、每条候选消费一次随机决策、只剩一条候选时不消费，
  按抽到的卡牌定义各创建 1 张新实例。只读定义、不移动来源，
  也不产生带来源身份的公开事件，因此"以非公开形式加入手牌"不需要额外机制。
- **S-93 `transform <目标> into random card from <集合>`**：`CardEffect.CopySource`
  与 `CardID` 互斥；一次结算只抽一次随机数，多目标共用结果，来源为空时不变身。
- 完成 4 张：10802310 救世的英姿（复制对手手牌 1 张、减 1 费、抽 1 张）、
  10912120 温泉度假猴（入场曲与进化时各复制 1 张）、10514110 脚踩天穹的《倒吊人》·罗弗拉德
  （入场曲按连击加身材；进化时舍弃全部手牌并从对手牌组复制 5 张）、
  10602210 被侵略的世界（启动：把选中的手牌变成对手牌组随机 1 张的复制）。
  卡包 10008 因此收满。
...
- **S-86 玩家标量之间的比较**：`CompareCondition` 新增 `RightExpr`，比较的右侧可以是
  数值表达式，于是"若自己的主战者的生命值大于对手的主战者的生命值"写作
  `if own.life > oppo.life { … }`。条件形状校验改成"两侧都是条件可用的数值操作数"，
  括号里的比较不再被误当成条件自身的运算符。集合计数作左值时右值仍只接受整数。
- **S-41 数值相减**：新增 `DifferenceExpr`（`kind: "difference"`），语法是
  `count(A) - count(B)` 这样的二元减号、自左向右结合；字面量不能当操作数
  （`count(A) - 1` 会在检查阶段报错），负结果由取用方按零处理。
- **S-90 按卡牌 ID 的入场张数**：玩家新增 `entered` 记录（按入场顺序保存本场对战
  进入过战场的随从与护符，打出的卡牌、召唤的衍生体与变身进场都算，离场不移除），
  只作为计数集合使用：`count(own.entered other where card 10931110)`。
  顺带修好 `validCountSource` 不接受 `count(集合 other where …)` 的问题——
  这个形状此前只在条件里出现、没经过数值编码，所以一直没暴露。
- 完成 4 张：10851130 兔耳恶魔·莉蜜儿（入场曲与进化时都在生命值更高时召唤 2 个蝙蝠）、
  10811110 昔日的天秤·马龙（按双方随从数之差随机破坏）、
  10844110 反照的赤红·德莱克&亚瑞札特（X 为其他同名卡入场张数，X≥2 时进化；
  谢幕曲给纹章，纹章【吟唱_2】谢幕曲把 1 张同名卡以 2 费加入手牌）、
  10931110 沉溺的实验体（其他同名卡入场 5 张以上时 +3/+3）。
...
- **账本化**：新增 `scripts/condition_scope_dispositions.json`，每张候选卡记录
...
- **新原语 `pp N { … }`**：支付 N 点能量点的块，不够则整块跳过（与 `earthrite`/`necromancy`/`faith`
  同族）。解析、解码白名单与 `PayResourceEffect` 运行分支都补齐，用于
  「与本卡牌【融合】时，消耗 2 点能量点，…」（10934110）。
- 完成 10 张：10912310 绿伞会两大招牌（连击 3 改为发动所有模式）、10923110 波涛副船长、
  10931120 钥灵（进化时给手牌增幅 4 次）、10933110 陶醉的才女、10934110 万术的罪人·赛菲
  （含纹章：自己的沉溺的实验体入场时给疾驰）、10944120 禁牙的变貌·诺玛格达拉、
  10954120 伽罗塔德 对 泽特（手牌中生命 ≤12 时回合结束减费；模式一/二）、
  10962310 铭刻之约、10963110 誓言干部（入场曲的回复自身触发破坏敌方随从）、
  10914120 香风的变貌·飞燕（手牌中随使用卡牌减费、谢幕曲召唤自己）。
- 新增 16 个场景（`tests/10009/batch-100-basics.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1333 全绿；`go test ./...` 全绿；语料快照更新为 1333 个场景。
...
- **S-88 `accelerate N { … }`**：新打出模式。解析（`typed_ir.go`/`validate.go`/`strict_validate.go`）、
  编解码（`decode.go`/`test_decode.go`）、运行器（`commitAccelerate`）与测试 DSL/模拟器命令
  全部接上；语义是"付激奏费用 → 卡牌进"结算中"区域（**不进入战场、不发动入场曲**）→
  只结算激奏能力 → 按法术流程进墓场"。与 `enhance N { … }`（仍入场）区分开。
- 补齐上一批登记的 5 张：10671110 低劣的玩具（激奏 2）、10672110 拙劣的人偶（激奏 3）、
  10673110 愚劣的兵器（激奏 4）、10844120 金银绚烂（激奏 3：能量点上限 +1）、
  10901110 最古老的狱卒（激奏 1）。**S-88 关闭，"已完成"口径重新可信。**
- 新增 8 个场景（`tests/10006/batch-98-accelerate.wbotest`、
  `tests/10009/batch-98-accelerate.wbotest`），覆盖激奏三种效果、正常打出仍走本体、
  费用不足不能激奏、以及激奏不触发入场曲/回报监听。
- 全量回归：`check` 0 错 0 警；`test` 1312 全绿；`go test ./...` 全绿；语料快照更新为 1312 个场景。

### 批次 97（卡包 10009 第四批 13 张 + S-88 缺口登记）

- 完成 13 张：10901110 最古老的狱卒（`random … other <绑定>` 排除刚选中的随从）、
  10942120 龙人管家、10913310 绯岸橙醉（手牌中回合结束连击 3 减费）、10961210 咆哮圣堂、
  10972110 锻磨保镖（按专项核查口径：回复也在"牌组无重复"条件内）、10953110 激烈的副总长、
  10954110 伊斯坦戴德 对 玛尔奇盖特（含纹章）、10974120 弹哭的变貌·艾兹伊甸、
  10941110 碎裂的盗匪、10914110 烟管的罪人·曲千代、10903110 无我看守、
  10974110 束刃的罪人·卡特斯罗特（含纹章）、10932110 心醉的研究者。
- **新登记缺口 S-88：【激奏】（accelerate）打出模式**。全卡表里有 5 张带 `accelerate` 文本
  （10671110、10672110、10673110、10844120、10901110），其中前三张此前已被记为"已完成"——
  复查 `alt_modes` 后发现它们只实现了本体，**激奏部分从未实现**。激奏需要"以较低费用打出、
  只结算激奏能力、不进入战场也不发动入场曲"的独立打出路径，和 `enhance N replaces`（仍入场）
  不是一回事。这 5 张暂按"本体已完成、激奏缺失"记录，等实现该模式后补场景。
- 新增 14 个场景（`tests/10009/batch-97-basics.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1304 全绿；`go test ./...` 全绿；语料快照更新为 1304 个场景。
...
- 新增 7 个场景（`tests/10009/batch-95-last-turn-attack.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1285 全绿；`go test ./...` 全绿；语料快照更新为 1285 个场景。

### 批次 94（卡包 10009 第三批 11 张）

- 完成 11 张：10963210 神纹誓言（入场曲破坏所有随从；启动破坏自身抽牌回复）、
  10973110 铸铁亲信、10924110 逆行的罪人·巴巴洛丝（`reduce countdown … where card …`）、
  10964110 神纹的罪人·艾尔拉德（含纹章）、10971310 次元选定、10973310 白牙燐敛、
  10913110 枝叶大姐大、10952310 锁链相连（`enhance 4 replaces` 改为两次）、
  10922310 燃尽之缘（按打出时费用判断并给同名卡设费）、10923310 黄金时代、10972120 刀线客。
- 新增 16 个场景（`tests/10009/batch-94-basics.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1278 全绿；`go test ./...` 全绿；语料快照更新为 1278 个场景。

### 批次 93（卡包 10009 第二批 16 张）

- 完成 16 张：10971120 双子无人机少女、10922110 漩涡炮手、10921110 海域斥候、10952110 渊底上校、
  10952120 闪烁恶魔、10953310 驱逐死神（`grant … lastwords { summon copies of self; }`）、
  10933310 乌涅、10941310 幼龙闹脾气、10951120 幽冥中尉、10922120 凶猛指挥官、
  10932310 人性的爱、10942310 颚口之别、10972310 屈辱流放、10962110 纪律间谍、
  10971110 青锈小卒，外加 10951110（批次 91 已单独提交）。
- 新登记缺口：S-87 "上一回合中攻击过主战者"（10942110、10943110）——现在只有本回合的
  `own|oppo.attacked_this_turn`。
- 新增 16 个场景（`tests/10009/batch-93-basics.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1261 全绿；`go test ./...` 全绿；语料快照更新为 1261 个场景。

### 批次 92（导入卡包 10009 + 首批 16 张）

- 导入最后一个卡包 10009 的 76 张骨架（语料快照 834 → 910 张，**全部 904 张卡表已导入**）。
- 完成首批 16 张：10911120 金刚伙伴、10962120 奇迹独角兔、10921120 团结骑兵、
  10931310 过度反应、10951110 强袭的特攻队长、10951310 恐怖惊遇、10961120 鹿鹰兽、
  10911210 森林陷阱（对手随从入场时破坏该随从与自身）、10911110 发芽组员、10941120 洞窟龙、
  10924120 武皇的变貌·贝尔铁佐（`attack_limit 3`）、10902110 圣剑天使、10934120 破式的变貌·菲绫、
  10961110 条规成员、10912110 根延潜伏者、10921310 方阵（`enhance 6 replaces` 召唤 5 个）。
- 写测试时的三个坑：对手侧的动作在 `.wbotest` 里没有 `actor` 写法，需要借"为我方/对手召唤"
  的效果（这里用 10224120 在对手战场召唤骑士）来触发"对手随从入场"的监听；
  `if overflow` 要求最大能量点 ≥7，光设当前 pp 不够；打出随从/法术前要记得给对应一方 pp。
- 新增 16 个场景（`tests/10009/batch-92-basics.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1245 全绿；`go test ./...` 全绿；语料快照更新为 910 张卡 / 1245 个场景。

### 批次 91（S-85 无视守护 + 10851110）

- **S-85 `ignore_ward`**：新固有关键词"可以无视【守护】进行攻击"。
  攻击合法性检查的两处守护限制（攻击主战者、攻击非守护随从）现在都会
  在攻击方带 `ignore_ward` 时放行；关键词加入 `validKeyword` 与 `abilities` 白名单。
- 完成 10851110 通透的信念·安瑟珠（【疾驰】+ 无视守护）。
- 新增 3 个场景（`tests/10008/batch-91-ignore-ward.wbotest`）：自带关键词、越过守护打主战者、
  以及"没有该关键词时攻击主战者仍报 `ward_required`"。
- 全量回归：`check` 0 错 0 警；`test` 1229 全绿；`go test ./...` 全绿；语料快照更新为 1229 个场景。

### 批次 90（卡包 10008 第六批 11 张）

- 完成 11 张：10814120 冰晶剑士·伊芙（爆能强化全体 +1/+1；自己身材增加时限次加牌）、
  10823110 冲田总司（解禁回合入场即进化；攻击随从时按进化状态打 1/3 次）、
  10823310 盛大演出（战场 ≥2 张时"改为发动所有模式"）、10824120 荣光的继承者、
  10833110 玛纳利亚文书官·琪可（含纹章：使用玛纳利亚法术时打主战者 1）、
  10834110 苍奏的继承者（增幅 X 的 10/20 两档加牌）、10852310 四重惊喜（`mode 2` 一次选两个）、
  10854110 欢乐的炼金术士（入场与进化各一个模式）、10863210 祥和的教会、
  10864120 希望的光彩·莉迪耶尔（含纹章）、10874110 决断的交错·亚修雷&莉缇雅。
- 测试侧再次确认：纹章吟唱在**控制者回合开始**时递减，所以想用 `end_turn` 触发到期，
  要把纹章放在对手身上（与既有 10000 卡包的纹章到期测试同一手法）；
  `advance turn_start own;` 不会走纹章吟唱。
- 新增 19 个场景（`tests/10008/batch-90-basics.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1226 全绿；`go test ./...` 全绿；语料快照更新为 1226 个场景。

### 批次 89（卡包 10008 第五批 6 张 + 筛选式减费）

- 完成 6 张：10844120 金银绚烂·璐米欧儿&雅尔贞特（舍弃 2 张手牌后打击敌方全体与主战者，
  超进化抽 3 张）、10833310 钢铁的小憩（X≥5 时同时回复生命与能量点——按英文与 SWB-RL 的口径
  放在同一条件下，即专项核查里记过的"同形不同解"之外的第三类）、10824110 天命的子弹·
  巴妮&巴隆、10803310 传承的意志（三选一模式）、10834120 灾难言灵·洋荷（入场监听里
...
### 批次 88（卡包 10008 第四批 14 张 + 三处语言扩展）

- 完成 14 张：10852120 黑暗骑士·玛莎、10874120 你的前辈·欧丝、10812110 太古的妖精·露芙蕾、
  10843310 狐火蜃景（可指定随从或主战者的混合目标 + 把复制加入牌组）、10862310 威胁的残渣
  （`enhance 5 replaces` 改为全体）、10832320 伏地雷击、10831110 森绿的恩惠、10873310 开辟未来、
  10804110 阿尔比昂巴哈姆特、10842120 满面笑容的烹饪、10822110 越狱者·卡婕、10862110 天阳的使徒·
  艾迪特、10863110 圣洁驱魔人·珂蕾特、10864110 飞跃的姐妹（含纹章）。
- 三处语言扩展：
  1. **宣告攻击的监听新增 `defender` 绑定**（只在攻击随从时有值）：`if count(defender) >= 1`
     用来表达"攻击随从时"，自动覆盖"攻击主战者时不发动"。Go 单测
     `TestAttackListenerBindsDefender` 固定形状。
  2. **筛选器接受 `type crest` / `type faith`**（只出现在主战者区域）：阿尔比昂巴哈姆特的
     "消灭所有纹章"按官方 QA 写作 `destroy own.crests where not type faith`，信仰不会被消灭。
  3. **`self` 的身材增减事件允许 `once per`**：太古的妖精写
     `when self stats increased once per own turn`（此前只有 `self survives damage` 能带限次）。
- 新登记缺口：S-86 玩家标量之间的比较（10851130 兔耳恶魔需要"自己的生命值 > 对手的生命值"，
  现在条件右侧只接受整数）。
- 新增 22 个场景（`tests/10008/batch-88-basics.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1197 全绿；`go test ./...` 全绿；语料快照更新为 1197 个场景。

### 批次 87（卡包 10008 第三批 12 张）

- 完成 12 张：10841130 沧海之精（纹章：自己的海洋·随从入场时 +1/+1）、
  10853310 改变的流向、10821120 寡言的刺客·夏伊莉、10842110 闪耀旋律·莉芙&萝萝、
  10861110 图书室的魔女·莉莉尤姆（进化时抹掉能力再打 2，`remove all abilities from target`）、
  10821130 悠久的骑士·莎夏、10814110 离合有终·赛德斯&梅希亚、10843110 穹顶的战火·杰亚达
  （攻击随从时获得屏障并变成 1 回合 2 次攻击）、10853110 诚实的诅咒师·丝姬、
  10821110 武力与治安·娜哈特&宾森特、10813110 温柔读心者·米榭儿（给全体随从屏障）、
  10822310 重历新生（攻击力最大者 -10/-10 并被破坏）。
- 写测试时再次确认两条既有规则：结果随机位置插入 → 牌组原本为空时才能确定性地验证
  "放回后再抽回"；生命值被减到 0 以下的随从会被破坏（断言 zone 而不是负数身材）。
- 新增 14 个场景（`tests/10008/batch-87-basics.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1175 全绿；`go test ./...` 全绿；语料快照更新为 1175 个场景。

### 批次 86（卡包 10008 第二批 15 张）

- 完成 15 张：10812310 宁静的孤独、10872310 纯净无垢的日常、10842310 末世死化妆、
  10851120 可爱恶魔·莉莉姆（攻击时打击双方主战者）、10862120 深渊探究者·维切、
  10871120 过度守护者·莉欧娜、10852110 母爱恶魔·菲欧蕾、10871110 满溢的幸福·
  库伦特司（谢幕曲召唤一个失去谢幕曲的自己，`remove lastwords from summoned`）、
  10841110 狼人族首领·契特、10873110 知恩图报·米莉亚姆、10802110 激动的欢喜·阿尔菲德、
  10861130 亡灵猎人·格兰特、10813310 水镜的信赖、10832110 快乐绽花·萨米&玛莉
  （`draw 1 for oppo`）、10831310 暴风破（`counter x 2` + 魔力增幅 +1）。
- 注意"主战者当前生命值"的条件写法是 `if own.life <= 10`（`own.<标量> 比较 整数`），
  不能写 `own.leader.life <= 10`——后者是数值位置才认的写法。
- 新增 19 个场景（`tests/10008/batch-86-basics.wbotest`）。
- 全量回归：`check` 0 错 0 警；`test` 1161 全绿；`go test ./...` 全绿；语料快照更新为 1161 个场景。
...
- **复核结论**：批次 73 之后新实现的 10704110、10754120、10763110、10753110、10823110、
  10833310、10863210、10943110、10972110、10554120 全部与各自文本口径一致，
  **没有发现新的条件范围错误**。
- **并入判定（统一口径）**：官方英文把后续句子并进条件句时按"条件内"实现，
  日/中/韩用句号切断只当排版。落在这一条上的是 10153110（【屏障】）、10763110（【屏障】）、
  10972110（回复4点）、10833310（回复能量点）、10754120（亡者入场打1点），
  以及日文原文同样在条件内的 90064320（加入天书深渊）。
  反向不成立：10372110／10372210／10653110／10753110 的条件是"是否做出了选择"，
  官方 QA 与英文都支持后续句独立结算，保持原实现（见上面的"同形不同解"）。
