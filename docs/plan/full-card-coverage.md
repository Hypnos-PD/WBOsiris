...
| S-76 | ~~再次发动自身【入场曲】~~ **已解决**：`replay fanfare self;` | 已解锁 10604110 恐惧的象征·欧米伽奥提普 | 运行期按实例当前的卡牌定义重新执行 `fanfare` 能力（新的效果帧，重新随机）；`fanfareReplays` 计数随实例快照保存，上限 12 次防止随机自引用无限递归。 |
...
| S-97 | 回合开始时"牌组中发动"与场上吟唱谢幕曲的先后 | 10404110 与【吟唱_1】护符同时在场时的结算顺序 | 官方 QA：场的倒计数破坏 → 瞬念召唤 → 该护符的谢幕曲 → 返回手牌 → 回合开始的抽牌。当前实现把倒计数破坏（含谢幕曲）放在同一阶段先结算完，再处理牌组中的回合开始能力，因此"瞬念召唤"排在该谢幕曲之后。需要把死亡批次的谢幕曲延后到牌组能力之后再结算。 |
...
- **新事件 `mode_selected`**：玩家确认模式选择后按每个选中的选项各派发一次，
  写作 `when own|oppo mode selected { … }`（`random` 模式不派发）。
...
- **S-76 `replay fanfare self;`**：按实例当前的卡牌定义查找 `fanfare` 能力，作为新的效果帧
  ...
  并随实例快照保存，上限 12 次：卡牌可以用随机模式自引用（10604110 的能力(4)），
  真实对局里长链概率迅速衰减，引擎需要一个确定性上限避免无限递归。
...
- 官方 QA 落实三点：① 只有瞬念召唤才获得纹章（正常打出不给）；② 瞬念召唤后本卡牌
  返回手牌；③ 纹章的回合开始能力先于牌组中发动的能力结算（本实现符合，只有
  "场上吟唱谢幕曲与瞬念召唤的相对顺序"仍与 QA 不同，登记为 S-97）。
