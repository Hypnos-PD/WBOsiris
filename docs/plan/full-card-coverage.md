...
```text
总计 904 · 已完成 283 · 未实现(骨架) 90 · 未导入 531
卡包      总数  已完成  未实现  未导入
10000        56      56       0       0
10001       142     142       0       0
10002        77      36      41       0
...
| S-10 | 纹章（crest）效果文本来源 | **63 张授予纹章的卡**（10204120、10232110、10264120、10243110、10254120、10214110、10233110…） | `crest { }` 块**必须**包含五语言文本，而我们的卡表只写了"使自己获得『纹章：X』"，**没有纹章自身的效果文本**（官方 QA 也只引用纹章、不给全文）。`../SWB-RL` 有 26 条结构化纹章定义（如 `hurricane_heavenly_grimnir`：自己回合结束时，若自己有超进化随从，则对敌方所有随从造成 2 点伤害），只能作为**假设**；按权威契约不能据此编写展示文本。需要你决定：① 提供纹章文本来源（游戏内数据/日文 wiki 等）；② 允许先用 SWB-RL 的语义 + 我们自拟文本并标注"待核对"；③ 继续挂起。 |
