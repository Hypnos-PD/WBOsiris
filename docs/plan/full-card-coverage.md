...
| S-88 | 【激奏】（accelerate）打出模式 | 10671110 低劣的玩具（激奏：召唤 1 个自己）、10672110 拙劣的人偶（召唤 2 个自己）、10673110 愚劣的兵器（召唤 1 个自己）、10844120 金银绚烂·璐米欧儿&雅尔贞特（能量点上限 +1）、10901110 最古老的狱卒（随机随从 2 点伤害） | 这 5 张卡在 `../WBArts` 的 `alt_modes` 里各有一条 `accelerate` 文本（五语言齐备），但 WBO 目前**没有激奏打出模式**：只实现了本体。激奏需要"以较低费用打出、只结算激奏能力、不进入战场/不发动入场曲"的独立打出路径（与 `enhance N replaces` 不同，后者仍会入场）。**覆盖率口径**：这 5 张目前只算"本体已完成"，激奏部分缺失，等实现后补场景。 |
