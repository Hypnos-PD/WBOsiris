...
```text
总计 904 · 已完成 648 · 未实现(骨架) 25 · 未导入 231
卡包      总数  已完成  未实现  未导入
10000        56      56       0       0
10001       142     142       0       0
10002        77      77       0       0
...
10009        76       0       0      76
90000        93      92       1       0
未完成卡按文本复杂度：中(41–100字) 251 · 短(≤40字) 134 · 长(>100字) 16 · 白板 1
```
...
| S-35 | ~~信仰值（faith）机制~~ **已完成主体**：`faith { counter value 0; … }` + `faith N { … }` + `distribute faith <卡牌ID> { … }` | 已解锁 10634120 古旧天晶、10664120 古旧天书、90034330 天晶深渊 | 信仰实体与纹章共用结构/区域，开局按初始牌组放置，信仰值用 `counter value` 表示；`faith N { … }` 是"信仰值-N 后执行"的支付块（不足则整块跳过）；`distribute faith` 把信仰值逐点随机分配给若干能力（不消费）。**待续**："使自己的信仰获得「…」"（10614120、10624120、10354110）。 |
