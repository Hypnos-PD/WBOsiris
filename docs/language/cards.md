...
"自己选择【模式】时"写成 `when own|oppo mode selected { … }`：一次"选择【模式】"的动作派发一次，
即使同时选中多个选项也只算一次（官方 QA qrpt1xyqmvgf）。
...
`set empty deck own|oppo victory;` 改变该玩家牌组耗尽时的结果：正常规则下抽不到牌即败北，
而"胜利的卡牌"让该玩家改为胜利（10304110 的纹章把牌组底部的亡者之王卡牌变成胜利的卡牌）。
用 `set empty deck own|oppo defeat;` 可以改回常规规则。

筛选词条 `where not card <卡牌ID>` 用来排除指定卡牌定义，
例如纹章的"舍弃除『绝大的证明』以外的所有手牌"写作
`discard own.hand where not card 90004320;`。
...
（选择数量、伤害量等）按零处理。
求和可以只取最高的/最低的 N 张：`sum(集合 [where 筛选], base.cost) highest 3`
（`lowest 3` 同理）。不足 N 张时按现有张数求和；同值时取任意 N 张（和相同）。
用于"自己的手牌中原始费用最大的 3 张卡牌的费用合计"这类文本。
...
只有当剩余能量点**不足以支付本卡牌当前的费用**时才能用【激奏】（官方 QA
npml0tn71d：有 5 点能量点时不能对 5 费的卡使用【激奏_3】）。
...
「回合结束时」的能力，其条件在**按下回合结束按钮时**统一判定（官方 QA
0stcr3cyb、zov-4dbr6tlx、75wdwp8ysf）：因此卡面写作"自己的回合结束时，若 X，则 Y"时，
条件要写在触发上（`when own turn ends if X { Y }`），而不是放进 `{ … }` 里，
否则回合结束阶段里被其它能力改动的状态会让它补发。

触发入队时会记下来源实例当时所在的区域，结算时来源已经离开该区域的触发不再发动
（官方 QA 2hv4yt1j8）。谢幕曲在破坏之后才入队，属于不做该检查的例外。

失去能力（`remove lastwords from …` / `remove all abilities from …`）只影响之后的能力查询，
**已经入队**的触发照常结算完（官方 QA mxtwkh_pd）。
...
写作 `if own.life > oppo.life { ... }`。两侧都可以是集合计数或求和表达式，
例如"自己的手牌中原始费用最大的 3 张费用合计大于对手的"写作
`if sum(own.hand, base.cost) highest 3 > sum(oppo.hand, base.cost) highest 3 { ... }`；
`count(A) > count(B)` 同样成立。
...
纹章还可以声明"持续性规则改动"，写作 `passive <名字>;`（只允许出现在纹章块）：

```wbo
crest {
    passive suppress_fanfare;   // 自己的随从的【入场曲】不发动
    passive suppress_enhance;   // 自己的随从的【爆能强化】不发动

    // "自己使用的随从入场时进化"用打出监听表达（效果引起的进化不发动进化时能力）。
    when own card played where type follower {
        evolve played silent;
    }
}
```

`suppress_fanfare` 只压制消耗点数触发的入场曲；`when self summoned` 这类入场监听不受影响。
`suppress_enhance` 只压制【爆能强化】档的效果；如果该卡牌写的是"改为"档，支付了档位费用
也不会替换原本的打出效果与入场曲（改动被压制，替换也不发生）；被压制时按**基础费用**支付
（官方 QA 4gt8z-3dnxk：有 4 点能量点时打出 2 费的【爆能强化_4】随从只消耗 2 点）。
两种压制都只作用于**随从**，法术与护符的爆能强化照常发动。
