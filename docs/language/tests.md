...
命名计数器可在实例初始状态中覆盖，例如 `spell source = 10131320 { counter x 7; }`，
...
`damage_taken N;` 可为随从预置尚未回复的伤害。先应用 `stats`（或卡牌默认数值），
再扣除该伤害，不发出伤害事件、不触发监听，也不消费屏障。数值必须非负且小于初始生命，
同一实例不能重复声明。例如 `stats 8/6; damage_taken 4;` 表示攻击 8、当前生命 2、生命上限 6。
单独的 `stats 8/2;` 表示生命与上限均为 2，不表示受伤。

`expect` 中的 `source.stats == 0/4;` 断言当前规则身材，攻击力以零为下限。
效果可以把内部攻击力降到负数；后续增益仍从负数计算，而场景断言与网页显示保持一致。

`cost` 可覆盖当前费用，范围为 0 到 65535，包括已经减到零费的实例：

```wbotest
spell source = 10134310 { cost 5; }
```

在 `expect` 中使用 `source.cost == 5;` 检查当前费用，实例移动区域后仍可断言。
...
`choose` 时，不产生玩家选择请求；效果以空目标继续结算。`require` 候选不足声明
