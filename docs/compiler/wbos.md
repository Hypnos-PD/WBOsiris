...
不保存 Go 内存引用。当前 Continuation 版本为 `0.18.0`，完整保存卡牌命名计数器、模式选项标签、事件可见范围、本回合融合状态、攻击次数、入场等待、攻击阶段、
...
产生的待处理谢幕曲。非法玩家键和两项均为零的冗余记录均拒绝恢复。运行器拒绝
`0.17.0` 及更早的续局存档；已有网页录像保存的状态帧仍可浏览。
