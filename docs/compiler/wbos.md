...
不保存 Go 内存引用。当前 Continuation 版本为 `0.16.0`，完整保存卡牌命名计数器、模式选项标签、事件可见范围、本回合融合状态、攻击次数、入场等待、攻击阶段、
...
`0.16.0` 将筛选定量抽牌改为按实例等概率抽选，并记录抽牌数量事件。由于 RNG 推进
语义改变，运行器拒绝 `0.15.0` 及更早的续局存档；已有网页录像保存的状态帧仍可浏览。
