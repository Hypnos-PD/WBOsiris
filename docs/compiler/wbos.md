...
不保存 Go 内存引用。当前 Continuation 版本为 `0.26.0`，完整保存带回合归属的独立破坏记录、每回合触发次数、卡牌命名计数器、模式选项标签、事件可见范围、本回合融合状态、攻击次数、入场等待、攻击阶段、
...
`0.16.0` 将筛选定量抽牌改为按实例等概率抽选，并记录抽牌数量事件。
`0.17.0` 增加 `endingSide` 与各实例的 `temporaryKeywords`，保存关键词的绝对玩家到期
标志和永久赋予标志。结束阶段暂停时不会提前到期，恢复后先完成触发队列再统一清理。
未知关键词、没有到期标志、与当前能力不一致或不合法的结束阶段均拒绝恢复。
`0.18.0` 增加实例的 `temporaryStats`，按绝对玩家保存临时属性增量，并保留到期死亡
产生的待处理谢幕曲。非法玩家键和两项均为零的冗余记录均拒绝恢复。
`0.19.0` 增加实例的 `grants`，按赋予顺序保存卡包内的附加能力节点 ID。
未知引用和非随从的赋予记录拒绝恢复；重复引用保留叠加次数，执行栈仍引用稳定的能力体。
`0.20.0` 增加攻击状态的 `defenderDestroyed`，保存本次攻击中防守随从是否实际被破坏，
超进化追伤不再将返回手牌或消失视作破坏。`opponent` 随普通绑定帧保存。
`0.21.0` 记录 `follower_left` 事件，恢复手牌监听索引及融合本体能力的结算边界。
`0.22.0` 将绑定帧的值由实例 ID 数组改为目标数组：实例保存为
`{ "kind": "instance", "instanceId": "..." }`，主战者保存为
`{ "kind": "leader", "side": "own" }` 或 `oppo`。此处席位为对局固定席位。
恢复时校验目标身份、候选类型、重复项及候选顺序，并重新计算待选择节点的合法目标。
旧版续局存档不兼容，应使用匹配其版本的引擎恢复。
`0.23.0` 支持连续独立必选目标及单次批次破坏；在第二次选择暂停时保留第一次
选择的绑定，并恢复到同一破坏节点。该版本同时收紧条件分支后的前置查询检查。
`0.24.0` 在实体中增加 `usedTriggers`，以印刷能力 ID 为键，仅保存值为 `true` 的已用记录。
恢复时拒绝未知能力、非限次能力、值为 `false`、回合范围不符或缺少已用记录的排队能力。
在监听入队后、执行前或能力内部暂停，均保留原有次数与目标绑定。
`0.25.0` 将玩家的 `destroyed` 从实例 ID 数组改为独立记录数组。每条记录包含
`instanceId`、`cardId`、`cost`、`attack`、`life`、`evolved`、`superEvolved`、
可选的 `departed` 和 `eventSequence`。实例 ID 可重复，卡牌 ID 可与存续实例不同。
`eventSequence` 为零表示测试初始历史；其他记录按事件顺序保存，必须与对应破坏事件的
席位、实例 ID 和卡牌 ID 一致，每个破坏事件恰好对应一条记录。
恢复时校验历史归属、卡牌定义、记录顺序与事件对应关系，保留每次破坏的独立抽选权重。
`0.26.0` 为破坏记录增加 `turnSide` 与 `turnNumber`，记录破坏发生时的行动方与回合号。
`eventSequence` 为零的测试初始历史不带回合归属；实际破坏必须具有有效且不晚于当前回合的
归属。回合开始阶段的倒计时破坏使用新回合，不依赖 `turn_started` 事件是否已经发出。
这保证历史求和在入场监听或回合开始阶段暂停、恢复后仍按同一回合取值。
运行器拒绝 `0.25.0` 及更早的续局存档；已有网页录像保存的状态帧仍可浏览。
