...
不保存 Go 内存引用。当前 Continuation 版本为 `0.17.0`，完整保存卡牌命名计数器、模式选项标签、事件可见范围、本回合融合状态、攻击次数、入场等待、攻击阶段、
...
未知关键词、没有到期标志、与当前能力不一致或不合法的结束阶段均拒绝恢复。运行器拒绝
`0.16.0` 及更早的续局存档；已有网页录像保存的状态帧仍可浏览。
