...
不保存 Go 内存引用。当前 Continuation 版本为 `0.27.0`，完整保存纹章区域、带回合归属的独立破坏记录、每回合触发次数、卡牌命名计数器、模式选项标签、事件可见范围、本回合融合状态、攻击次数、入场等待、攻击阶段、
...
来源定义和事件归属。运行器拒绝 `0.26.0` 及更早的续局存档；已有网页录像保存的
