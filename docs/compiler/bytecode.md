...
的版本化 Continuation、确定性执行预算和 FIFO 触发队列覆盖现有测试纵向切片。它尚未
实现 TriggerIndex、死亡批次和全部规则动作，因此不构成字节码规范；字节码语义仍以语言规范、
类型化 IR 和 MatchRuleset 为准。
...
当前 IR 解释器的 `Continuation 0.2.0` 使用稳定块 ID 保存每个根效果块和嵌套分支，
...
跨进程 Continuation、沙箱预检与 FIFO 触发队列已经完成。后续顺序为：

1. 完成 TriggerIndex 和死亡批次。
2. 实现字节码验证器及文本汇编表示，建立测试向量。
3. 将完整规则场景同时运行于 IR 解释器和字节码 VM，做差分验证。
4. 语义稳定后分配 opcode 数字并设计 WBOS 二进制分区。
