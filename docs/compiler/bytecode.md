...
当前 IR 解释器的 `Continuation 0.16.0` 使用稳定块 ID 保存每个根效果块和嵌套分支，
并保存共享绑定帧、栈 PC、当前及待处理触发、请求候选、实例、区域、事件、RNG、实例
计数器、攻击次数、入场等待、玩家回合攻击历史、攻击阶段、正在结算的法术和状态修订。编码锁定卡牌包哈希与默认
规则集哈希；恢复时重新建立块索引，并
拒绝块、节点、候选、绑定、卡包、规则集或修订不匹配。JSON 解码拒绝未知字段、重复键、
尾随数据和超过 1 MiB 的 Continuation。该解释器格式是当前运行时协议，不替代未来
字节码函数与基本块格式。运行器仅接受当前版本；卡牌命名计数器与模式选项的本地化标签也会保存，
恢复时逐项核对标签与 IR 节点，并继续验证融合、变身事件的 `privateTo` 可见范围。
`0.16.0` 固定筛选抽牌的随机语义，旧续局存档不能使用新的 RNG 推进方式恢复。
恢复不会重做进化或支付；会验证最小、最大选择数量与 IR 节点相符，并按灵气、潜行和形态筛选校验候选。

重复块的栈帧另保存 `repeatRemaining`（包含当前迭代）和 `repeatBindingsId`（外层绑定帧）。
恢复会验证循环调用节点、块 ID、来源实例、外层绑定及剩余次数；固定次数不能超过源码次数。
每次循环复制外层绑定，重复块结束后释放该局部帧。剩余次数不会根据恢复后的区域重新计算，
也不会通过重新执行已完成效果来重建。非重复块不得携带循环字段。

`Game.own.resolving` 与 `Game.oppo.resolving` 保存正在结算的法术实例 ID。它是执行器
内部区域，不是作者可声明的初始区域。整个对局最多有一个正在结算的法术，必须属于
当前行动方，绑定未完成的法术执行栈，并保持延后触发的检查点。法术完成时才进入
`graveyard`，增加一点 `shadows`，再执行等待的触发。恢复不得重新入墓、重新增幅或
重算墓场计数；实例当前费用为 `0` 时必须原样恢复，不能用印刷费用替代。

`choose` 的 optional 表示候选为空时绑定 `none` 并继续，不表示候选非空时玩家可以
跳过。`require` 候选不足声明数量时使整个动作在预检阶段非法。`random_choose` 不暂停，
不重复抽取最多声明数量的实例，每个选中实例消费一次规则随机决策。
...
使用法术产生的自动魔力增幅先记录手牌实例并入队，法术正文及生效的强化能力完成后
才进入触发队列检查点。正文中的选择只暂停原动作，不提前执行等待的增幅；恢复时
沿用原有队列和结算位置，不重新使用法术或增幅。正文中产生的后续触发排在等待队列末尾。
...
当前 IR 解释器已按 `wbo-standard-0.3.0.executionBudget` 对效果节点、查询访问、最大
