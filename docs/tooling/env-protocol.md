...
### 特权信息（`oracle: true`，训练专用）

`reset` 时传 `oracle: true`，每个 `state` 事件会额外带一个 `oracle` 块，里面是**玩家在
正常对局里看不到、但引擎知道的真值**。客户端与回放路径永远不会拿到这段（只在训练环境
里、显式开关下外发），用途是 oracle guiding / 信念建模：训练时用真值塑造表示，
推理时模型只能靠自己从公开信息里推断。

```json
{"cmd":"reset","seed":7,"opponent":"external","oracle":true}
{"type":"state","side":"own","view":{…},"legal":[…],
 "oracle":{"oppoHand":[10661110,10762210],
           "oppoDeck":[10001110,10951120,…],   // 完整顺序，顶部在前（40 张）
           "ownDeck":[10762210,10002110,…],    // 完整顺序（真人连自己的顺序都不知道）
           "oppoMaterials":{"3f2a…":[10001110]}}}   // 对手融合素材（对手视角被隐藏）
```

覆盖范围（与 `View` 的脱敏点一一对应）：

| 隐藏信息 | 公开视角给什么 | `oracle` 给什么 |
| --- | --- | --- |
| 对手手牌内容 | 只有张数 | `oppoHand`（卡牌 ID 列表） |
| 双方牌库顺序 | 连自己的都不给（只给"剩余内容多集"） | `oppoDeck` / `ownDeck`（完整全序） |

> 牌库那条以前**是漏的**：`view.own.deckRemaining` 原本按牌库遍历顺序外发，也就是
> "接下来会抽到什么"直接给出来了（训练侧只按多重集分桶所以没暴露，客户端拿它做提示
> 就是作弊）。现在它按卡牌 ID 排序后外发：同一副牌的任何顺序都得到同一个 view，
> 张数与内容不变。要真顺序只有一条路——特权视图的 `ownDeck`，只在训练时开。

| 对手融合素材 | 对手视角 `fusion` 为 null | `oppoMaterials`（instanceID → 素材 ID） |

刻意**不在**其中的：随机数状态。它同样是隐藏信息，但公开信息里几乎不可能推断，
作为信念目标只会注入噪声；需要"未来随机结果"的实验可以用同一 `seed` 复现整局。
对手的选择请求（target/mode）也不在这里——轮到对手决策时，`view.pendingChoice` 与
`choice` 本身就是公开的。

### 确定化克隆（`clone` + `determinize`）

```json
{"cmd":"clone","handle":0,"determinize":123456}
```

`determinize>0` 时，克隆出来的局面会先做一次**确定化**：把**只有隐藏信息**的部分重新抽样，
公开信息逐位不变。

- 重排的槽位：对手手牌内容（与对手牌库做联合置换）、对手牌库顺序、己方牌库顺序；
- 不动的：双方计数、场上/墓地/放逐/破坏/护符、己方手牌内容；
- 同一个种子 ⇒ 同一个确定化样本（可复现），且不影响原局面或原句柄。

用途：树搜索现在可以"用与真值同分布、但不知道具体是哪一种的状态"来模拟，
不再偷看对手的真手牌与真牌序。**默认不启用**（不传 `determinize` 就与以前逐位一致）。
