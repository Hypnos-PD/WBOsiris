...
```bash
wbo test --ruleset wbo-standard-0.3.0 --source-root . tests
```
...
当前执行器覆盖现有 15 个基础场景所需的打出、启动、超进化、基础攻击、模式/目标选择、
触发、资源、区域移动、回合结束准备、测试驱动的 `advance` 和断言。攻击关键词、普通进化
及融合玩家指令已进入执行器范围。
...
场上实体通过 `attacksUsed` 和 `summoningSick` 公开当前攻击次数与入场等待；双方玩家状态
通过 `attackedThisTurn` 公开本回合是否已有随从攻击。初始状态直接位于战场的随从默认没有
入场等待，通过打出或召唤进入战场的随从则等待到其控制者的下一个回合开始。
...
当前模拟接口正式支持打出、启动、进化、融合、超进化、基础攻击、结束回合、目标选择和模式选择。
普通未定义的动作会返回 `unsupported_feature`，不会用未成文规则猜测结果。进化和融合完成后会
