...
实验过程中实测到一次**破坏性副作用**：客户端与清单相关的链路对不上时，会**删掉自己
下载的资源缓存**。本机上 `ShadowverseWB_Data/Persistent/dat/` 从 38265 个文件
（约 27 GiB 里的那部分）掉到 32 个文件（168 MiB），目录 mtime 正好是实验当刻。
之后它就会弹自己的「下载数据：1.79GB」对话框——那是它重新要资源，不是我们伪造的。

**给后来者的提醒**：在这个目录上做实验之前先备份 `Persistent/dat/`，或者清楚
"客户端可能把它清掉、需要连官方服务器重新下"的代价。这个目录是可再生的（重新跑一次
正常游戏即可），但别在没网的时候把它当成不可再生的资产。
